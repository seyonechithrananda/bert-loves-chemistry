...
def load_molnet_dataset(name: str, split: str = None, tasks_wanted: List = None, df_format: str = "chemberta"):
    ...
    tasks, splits, transformers = load_fn(featurizer="Raw", split=split or MOLNET_DIRECTORY[name]["split"])
    ...
    return tasks_wanted, [make_dataframe(s, MOLNET_DIRECTORY[name]["dataset_type"], tasks, tasks_wanted, df_format) for s in splits], transformers
...
def write_molnet_dataset_for_chemprop(name: str, split: str = None, tasks_wanted: List = None, data_dir: str = None):
    ...
    tasks, dataframes, transformers = load_molnet_dataset(name, split=split, tasks_wanted=tasks_wanted, df_format="chemprop")
